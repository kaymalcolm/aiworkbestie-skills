...
Generate a complete multi-platform content package for Kay Malcolm's "AI For You" series. Accepts a research file (preferred) or a topic directly. Always runs to a full draft — no research-only mode. For research-only, use `/content-kit-research`.
...
- `POST_NUMBER`: Count files in content-drafts matching `content-kit-0*.md`. Confirm this is accurate before proceeding.
...
After all content is drafted, run `/content-kit-grader` on every piece of platform content before saving any files. Do not write a single file or touch the DB until every section passes. Fix inline, then re-grade.
...
Run `/content-kit-register [POST_NUMBER]` after all files are saved. The register skill reads the saved files and writes all DB rows (posts, assets, destinations, scripts, slides, captions, keywords, production, queue position). Do not proceed to STEP 7 until registration is confirmed.
...
- Offer: "Want me to generate the infographic now?" (uses the `/content-kit-infographic` skill)
...
- Does NOT generate the infographic (offers to, then hands off to `/content-kit-infographic`)
- Does NOT register the draft in the DB (hands off to `/content-kit-register`)
...
Flag it at the top of the output: "Note: content-kit-002 covered [related topic]. This post builds on that by focusing on [specific angle]."
