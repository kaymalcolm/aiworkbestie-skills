...
**Explicit:** `/ai-for-you-manychat [POST_NUMBER]`
...
This skill runs AFTER `/ai-for-you-substack`. It needs:
- The comment keyword (registered in `post_keywords` during `/ai-for-you`)
- The Substack URL (registered in `destinations` or derivable from the slug after `/ai-for-you-substack` runs)
