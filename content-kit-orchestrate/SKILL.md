...
**Explicit:**
- `/content-kit-orchestrate "[topic]"` — start from Phase 1 (full pipeline)
- `/content-kit-orchestrate [research file path]` — start from Phase 2 (newsletter onward)
- `/content-kit-orchestrate [newsletter file path]` — start from Phase 3 (social content onward)
...
```
Phase 1:  /content-kit-research   → research file
          ↓ PAUSE 1 — reel + hooks review
Phase 2:  /content-kit-newsletter → newsletter file   [auto-advance]
Phase 3:  /content-kit            → social content package
          ↓ PAUSE 2 — B-roll direction review (inside Phase 3)
Phase 4:  ┌─ /content-kit-infographic  [parallel agents]
          └─ /content-kit-story
          ↓ [wait for both agents to complete]
          /content-kit-brief      → production brief (auto-generated)
          ↓ PAUSE 3 — infographic review before Substack publish
Phase 5:  /content-kit-substack-publish → brief updated with Substack URL
          /content-kit-manychat
```
...
Invoke the `/content-kit-research` skill with the provided topic. Run it to completion. The skill saves the research file and reports back the file path, proposed slug, keyword candidate, B-roll direction, and hook options.
...
Invoke the `/content-kit-newsletter` skill with the research file path. Run it to completion. The skill registers the keyword, creates the post record, generates the newsletter, and saves the newsletter file.
...
Invoke the `/content-kit` skill with the newsletter file path. The skill handles:
...
**Agent A — Infographic:**
```
Prompt: "Read and execute the skill at /Users/kmalcolm/.claude/skills/content-kit-infographic/SKILL.md.
...
**Agent B — Stories:**
```
Prompt: "Read and execute the skill at /Users/kmalcolm/.claude/skills/content-kit-story/SKILL.md.
...
Immediately after Phase 4 completes (both agents done), invoke `/content-kit-brief [POST_NUMBER]`. Run it to completion. The skill registers the brief in the DB, saves the file, and uploads to Drive.
...
Ready for Phase 5:
  → Publish to Substack:  /content-kit-substack-publish [NEWSLETTER_FILE_PATH]
  → Set up ManyChat:      /content-kit-manychat [POST_NUMBER]

Run them in order — ManyChat config depends on the published Substack URL.

After Substack publishes, run /content-kit-brief [POST_NUMBER] again to update
