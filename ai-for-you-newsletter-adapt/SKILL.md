...
**Explicit:**
- `/ai-for-you-newsletter-adapt [source URL or file path] [post number]`
- `/ai-for-you-newsletter-adapt [post number]` — if source URL is already in the plan file
...
```
### COVER INFOGRAPHIC
*[Cover infographic: generated via /ai-for-you-infographic [POST_NUMBER] cover]*
```
...
The visual spec in the mid placeholder IS the brief the `/ai-for-you-infographic` skill reads. Write it with enough detail that the illustration prompt can be generated from it. Match the scene to the newsletter's emotional moment — not generic "woman at laptop."
...
Run `/ai-for-you-grader` on each section before saving. Fix any section that grades below A+. Maximum 3 fix iterations per section.
...
```
/ai-for-you-infographic [POST_NUMBER] cover
```
...
- Next steps: `/ai-for-you-infographic [POST_NUMBER]`, then publish
