...
**Explicit:** `/ai-for-you-substack-publish [post number]` or `/ai-for-you-substack-publish [path to newsletter file]`

**Legacy alias:** `/ai-for-you-substack` also activates this skill.
...
If neither query returns a row, stop and tell the user no infographics with Drive records were found for this post — they may need to run `/ai-for-you-infographic [POST_NUMBER]` first. Use `mid_path` as IMAGE_1_PATH and `dl_path` as IMAGE_2_PATH in STEP 1.
...
if not existing:
    ...
    cur.execute("""
        ...
        VALUES ('substack-draft','ai-for-you',:1,:2,'ready',:3,:4,:5,:6)
