...
**Explicit:** `/content-kit-newsletter [path to research file]` or `/content-kit-newsletter [post number]`
...
Phase 2: /content-kit-newsletter → newsletter file  ← this skill
Phase 3: /content-kit            → social content package (reads newsletter file)
Phase 4: parallel visual assets
Phase 5: /content-kit-substack-publish, /content-kit-manychat
```
...
- Extract from the research file:
  - `TOPIC`
  - `PROPOSED_POST_NUMBER` (verify by counting files in `content-drafts/` matching `content-kit-0*.md`)
...
Count files in `content-drafts/` matching `content-kit-0*.md`. Every 3rd post is a FULL CONTENT post; others are TEASE posts.
...
if not post_row:
    ...
    cur.execute(
        ...
        "VALUES (:1,:2,:3,'content-kit','DRAFT',:4) RETURNING id INTO :5",
...
Do not include a URL (the newsletter isn't published yet).  Come back and update once the newsletter is published in the /content-kit-substack-publish skill.

---

### STEP 4 — Run the brand grader

Before saving any files, run `/content-kit-grader` on:
...
The `## SUBSTACK EDITION` section is what `/content-kit-substack-publish` reads.
...
if not existing:
    ...
    cur.execute("""
        ...
        VALUES ('newsletter','content-kit',:1,:2,'DRAFT',:3,:4,:5,:6)
        RETURNING id INTO :7""",
        [slug + "-newsletter",
...
- Asset DB ID
- "Ready for Phase 3: `/content-kit [path to newsletter file]`"
